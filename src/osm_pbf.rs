// Automatically generated rust module for 'osmpbf.proto' file
...
pub struct HeaderBlock<'a> {
    pub bbox: Option<osm_pbf::HeaderBBox>,
...
impl<'a> MessageRead<'a> for HeaderBlock<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        let mut msg = Self::default();
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.bbox = Some(r.read_message::<osm_pbf::HeaderBBox>(bytes)?),
...
pub struct PrimitiveBlock<'a> {
    pub stringtable: osm_pbf::StringTable<'a>,
    pub primitivegroup: Vec<osm_pbf::PrimitiveGroup>,
    pub granularity: i32,
    pub lat_offset: i64,
    pub lon_offset: i64,
...
impl<'a> MessageRead<'a> for PrimitiveBlock<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.stringtable = r.read_message::<osm_pbf::StringTable>(bytes)?,
                Ok(18) => msg.primitivegroup.push(r.read_message::<osm_pbf::PrimitiveGroup>(bytes)?),
...
pub struct PrimitiveGroup {
    pub nodes: Vec<osm_pbf::Node>,
    pub dense: Option<osm_pbf::DenseNodes>,
    pub ways: Vec<osm_pbf::Way>,
    pub relations: Vec<osm_pbf::Relation>,
    pub changesets: Vec<osm_pbf::ChangeSet>,
}
...
impl<'a> MessageRead<'a> for PrimitiveGroup {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.nodes.push(r.read_message::<osm_pbf::Node>(bytes)?),
                Ok(18) => msg.dense = Some(r.read_message::<osm_pbf::DenseNodes>(bytes)?),
                Ok(26) => msg.ways.push(r.read_message::<osm_pbf::Way>(bytes)?),
                Ok(34) => msg.relations.push(r.read_message::<osm_pbf::Relation>(bytes)?),
                Ok(42) => msg.changesets.push(r.read_message::<osm_pbf::ChangeSet>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
impl<'a> MessageRead<'a> for StringTable<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        let mut msg = Self::default();
        while !r.is_eof() {
            match r.next_tag(bytes) {
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(34) => msg.info = Some(r.read_message::<osm_pbf::Info>(bytes)?),
...
pub struct DenseNodes {
    pub id: Vec<i64>,
    pub denseinfo: Option<osm_pbf::DenseInfo>,
    pub lat: Vec<i64>,
    pub lon: Vec<i64>,
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(42) => msg.denseinfo = Some(r.read_message::<osm_pbf::DenseInfo>(bytes)?),
...
pub struct Relation {
    ...
    pub memids: Vec<i64>,
    pub types: Vec<osm_pbf::mod_Relation::MemberType>,
}
